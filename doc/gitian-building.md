...
*Setup instructions for a Gitian build of LitecoinS Core using a Debian VM or physical system.*
...
- [Building LitecoinS Core](#building-litecoins-core)
...
Clone the git repositories for litecoins and Gitian.
...
in the litecoins repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building LitecoinS Core
----------------

To build LitecoinS Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the litecoins repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/litecoins-project/litecoins
...
For example:
```bash
URL=https://github.com/thrasher-/litecoins.git
...
and the litecoins git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../litecoins/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/litecoins
SIGPATH=/some/root/path/litecoins-detached-sigs

./bin/gbuild --url litecoins=${BTCPATH},signature=${SIGPATH} ../litecoins/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/litecoins-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/litecoins-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/litecoins-osx-build.assert
```
...
[litecoins-project/gitian.sigs.lts](https://github.com/litecoins-project/gitian.sigs.lts/) repository, or if that's not possible create a pull
