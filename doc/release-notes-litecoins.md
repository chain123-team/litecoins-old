LitecoinS Core version *0.15.1* is now available from:

  <https://download.litecoins.org/litecoins-0.15.1/>
...
A number of changes to the way LitecoinS Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 ltsn per kB (0.00001 LTS). This is to prepare for dropping the minimum transaction fee to 0.1 ltsn per kB (0.0001 LTS) in the next release.
