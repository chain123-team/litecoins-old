LitecoinS Core version 0.13.2 is now available from:

  <https://download.litecoins.org/litecoins-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/litecoins-project/litecoins/issues>
...
updates will be released anymore. Without security updates, using a litecoins
...
ECDSA signatures inside LitecoinS transactions now use validation using
...
Previous versions of LitecoinS Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of LitecoinS Core was to raise the default minimum
relay fee.

LitecoinS Core 0.13.2 will have a strict maximum size on the mempool. The
...
LitecoinS Core 0.13.2 also introduces new default policy limits on the length and
...
LitecoinS Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. LitecoinS Core will relay transactions with
...
entirely in LitecoinS Core 0.13, and it is at this time undecided whether the
...
LitecoinS Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
LitecoinS Core automatically creates a hidden service to listen on, without
manual configuration. LitecoinS Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if LitecoinS Core is listening, and
...
LitecoinSd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals LitecoinS
Core to use floating fees. By default, LitecoinS Core will use floating
fees.
...
(default: `0.0002` LTS/kB).

At all times, LitecoinS Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTS.
Furthermore, LitecoinS Core will never create transactions paying less than
...
`litecoins.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 211989 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 211989 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litecoinsrpc>
        ProxyPass http://127.0.0.1:11989/
        ProxyPassReverse http://127.0.0.1:11989/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass litecoinsd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< litecoinsrpc:<password>
    </Location>
...
Restarting litecoinsd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in litecoins.conf
...
litecoins-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/litecoins-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The LitecoinS Core code base has
...
- `litecoins-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `litecoins-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the LitecoinS scripting system that in combination with
...
LitecoinS transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of LitecoinS Core.
...
The two phases are distinct in the LitecoinS-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming LitecoinS wallet
  software more difficult and which seriously complicates the design of smart
  contracts for LitecoinS.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each LitecoinS block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of litecoinss they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of litecoinss being spent is exactly
...
- **Increased security for multisig:** LitecoinS addresses (both P2PKH addresses
  ...
  believed to be provided by LitecoinS's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of LitecoinS's current ledger (called the UTXO set),
  ...
  note that LitecoinS Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  LitecoinS users to individually opt-in to almost any change in the LitecoinS
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and LitecoinS Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the LitecoinS
client to LitecoinS Core 0.13.2 to signal for segwit support. When a
...
Since LitecoinS Core 0.10.0, nodes have defaulted to only relaying and
...
- `litecoins-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `litecoins-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
